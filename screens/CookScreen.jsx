...
export default function CookScreen() {
  // State variables
  const [currentStep, setCurrentStep] = useState('foodType'); // foodType, servings, recipes, details
  ...
  // Food type options
  const foodTypes = [
    { id: 'protein', name: 'Protein-Rich Foods', color: '#FF6B6B', icon: 'nutrition-outline' },
    { id: 'mineral', name: 'Mineral-Rich Foods', color: '#4ECDC4', icon: 'leaf-outline' },
    { id: 'vitamin', name: 'Vitamin-Rich Foods', color: '#FFD166', icon: 'sunny-outline' },
    { id: 'fat', name: 'Healthy Fat-Rich Foods', color: '#6B5CA5', icon: 'water-outline' },
    { id: 'carbs', name: 'Carbohydrate-Rich Foods', color: '#F9844A', icon: 'fast-food-outline' },
    { id: 'fiber', name: 'Fiber-Rich Foods', color: '#4D908E', icon: 'flower-outline' },
    { id: 'hydrating', name: 'Hydrating/Water-Rich Foods', color: '#277DA1', icon: 'water-outline' },
  ];
  ...
  const handleServingsChange = (value) => {
    setServings(Math.max(1, Math.min(10, value))); // Limit between 1-10
  };
  ...
  const renderRecipeList = () => (
    <View style={styles.stepContainer}>
      ...
      <Text style={styles.stepTitle}>Suggested Recipes</Text>
      <Text style={styles.stepDescription}>
        Based on your {selectedFoodType.name.toLowerCase()} and {servings} servings
      </Text>
            ...
            <View style={styles.recipeInfo}>
              ...
              <View style={styles.recipeMetaContainer}>
                <View style={styles.recipeMeta}>
                  ...
                  <Text style={styles.recipeMetaText}>{item.prepTime} mins</Text>
                </View>
              ...
              <TouchableOpacity 
                style={[styles.selectRecipeButton, { backgroundColor: selectedFoodType.color }]}
                onPress={() => handleRecipeSelect(item)}
              >
                <Text style={styles.selectRecipeButtonText}>Select This Recipe</Text>
              </TouchableOpacity>
            </View>
          </View>
        )}
          ...
          <View style={styles.recipeDetailOverlay}>
            ...
            <View style={styles.recipeDetailMeta}>
              <View style={styles.recipeDetailMetaItem}>
                ...
                <Text style={styles.recipeDetailMetaText}>{selectedRecipe.prepTime} mins</Text>
              </View>
              
              <View style={[styles.recipeDetailMetaItem, styles.recipeDifficultyBadge, 
                selectedRecipe.difficulty === 'Easy' ? styles.easyBadge : 
                selectedRecipe.difficulty === 'Medium' ? styles.mediumBadge : styles.hardBadge
              ]}>
                <Text style={styles.recipeDifficultyText}>{selectedRecipe.difficulty}</Text>
              </View>
              
              <View style={styles.recipeDetailMetaItem}>
                <Ionicons name="people-outline" size={18} color="white" />
                <Text style={styles.recipeDetailMetaText}>{servings} servings</Text>
              </View>
            </View>
          </View>
        </View>
        ...
        <View style={styles.recipeDetailContent}>
          <View style={styles.recipeDetailSection}>
            <Text style={styles.recipeDetailSectionTitle}>
              <Ionicons name="list-outline" size={20} color={selectedFoodType.color} /> Ingredients
            </Text>
            {selectedRecipe.ingredients.map((ingredient, index) => (
              <View key={index} style={styles.ingredientItem}>
                <View style={[styles.ingredientDot, { backgroundColor: selectedFoodType.color }]} />
          ...
          <View style={styles.recipeDetailSection}>
            <Text style={styles.recipeDetailSectionTitle}>
              <Ionicons name="restaurant-outline" size={20} color={selectedFoodType.color} /> Preparation Steps
            </Text>
          ...
          <View style={styles.recipeDetailSection}>
            <Text style={styles.recipeDetailSectionTitle}>
              <Ionicons name="bulb-outline" size={20} color={selectedFoodType.color} /> Cooking Tips
            </Text>
  ...
  const renderFoodTypeSelection = () => (
    <View style={styles.stepContainer}>
      <Text style={styles.stepTitle}>Select a Food Type</Text>
      <Text style={styles.stepDescription}>Choose the type of food you'd like to cook</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity 
            style={[styles.foodTypeCard, { backgroundColor: item.color + '20' }]}
            onPress={() => handleFoodTypeSelect(item)}
          >
            <View style={[styles.foodTypeIconContainer, { backgroundColor: item.color }]}>
              <Ionicons name={item.icon} size={24} color="white" />
            </View>
            <Text style={styles.foodTypeName}>{item.name}</Text>
            <Ionicons name="chevron-forward" size={24} color={item.color} style={styles.foodTypeArrow} />
          </TouchableOpacity>
        )}
  ...
  const renderServingsSelection = () => (
    <View style={styles.stepContainer}>
      ...
      <Text style={styles.stepTitle}>How Many Servings?</Text>
      <Text style={styles.stepDescription}>Select the number of servings you want to prepare</Text>
      
      <View style={styles.servingsContainer}>
        <View style={styles.servingsCard}>
          <View style={styles.blobWrapper}>
            <BlobBackground color={selectedFoodType.color + '40'} />
          </View>
          ...
          <TouchableOpacity 
            style={[styles.generateButton, { backgroundColor: selectedFoodType.color }]}
      ...
      >
        {loading ? (
          <View style={styles.loadingContainer}>
            ...
            <Text style={styles.loadingText}>Generating delicious recipes...</Text>
          </View>
        ) : error ? (
